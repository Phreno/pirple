 ...
 * @ Modified by: Phreno
 * @ Modified time: 2020-04-08 09:24:58
...
handlers._users.delete = (data, cb) => {
  ...
  if (phone) {
    ...
    handlers._tokens.verifyToken(tokenID, phone, (tokenIsValid) => {
      if (tokenIsValid) {
        _storage.read('users', phone, (err, userData) => {
          if (!err && userData) {
            _storage.delete('users', phone, (err) => {
              if (!err) {
                ...
                helpers.checkArray(userData.checks) &&
                  (checks = userData.checks)
                if (checks.length) {
                  checks.reduce((deletionErrors, checkID, id) => {
                    _storage.delete('checks', checkID, (err) => err && deletionErrors.push(checkID))
                    if (id + 1 === checks.length) {
                      if (!deletionErrors.length) {
                        cb(200)
                      } else {
                        cb(500, { Error: 'Error while deleting some user’s cheks', source: deletionErrors })
                      }
                    }
                    return deletionErrors
                  }, [])
                  checks.forEach(id => {
                    _storage.delete('checks', id, (err) => {
                      err && (deletionError = true)
  ...
  } else { cb(400, { Error: 'Missing required phone number' }) }
}
